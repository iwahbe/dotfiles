...
We don't actually want to tangle this file into its surrounding directory. We want to tangle it into the =home= directory.

#+BEGIN_SRC emacs-lisp
(advice-add #'org-babel-effective-tangled-filename :around
	    (lambda (fn buffer-fn src-lang src-tfile)
	      "Ensure that all files tangled from readme.org are emmited to the home directory."
...
Apple pre-appends to the user set path between running =.zshenv= and =.zshrc= via the =path_helper= (see =man path_helper=) subsystem. To ensure that user declared paths come before system supplied paths, we set =$user_path= to pre-append =$path= in the beginning of =.zshrc=.
...
#+BEGIN_SRC fundamental :tangle .global_gitignore
.#*     // To ignore emacs backup files
go.work // Go work files are dev tools, and should not be checked in.
#+END_SRC
